...
import com.boatit.boatsharing.ui.captain.dashbaord.model.VoyageCompleteResponse
...
class CompleteVoyageRepository(private val httpClient: HttpClient) {
    suspend fun status(profile: VoyageCompleteRequest): Result<VoyageCompleteResponse> {
        return try {
            val response: HttpResponse = httpClient.post("${ApiConstants.BASE_URL}${ApiConstants.Endpoints.COMPLETE_VOYAGE}") {
