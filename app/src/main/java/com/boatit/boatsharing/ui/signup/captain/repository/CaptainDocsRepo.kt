...
class CaptainDocsRepository(private val httpClient: HttpClient) {
    suspend fun CaptainDocs(profile: SaveCaptainDocumentRequest): Result<SaveCaptainDocumentResponse> {
        return try {
            val response: HttpResponse = httpClient.post("${ApiConstants.BASE_URL}${ApiConstants.Endpoints.SAVE_CAPTAIN_DOCS}") {
                contentType(ContentType.Application.Json)
                setBody(profile)
            }
            if (response.status == HttpStatusCode.OK) {
                val result: SaveCaptainDocumentResponse = response.body()
