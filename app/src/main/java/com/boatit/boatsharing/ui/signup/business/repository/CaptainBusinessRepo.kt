...
class BusinessProfileRepository(private val httpClient: HttpClient) {
    suspend fun BusinessProfile(profile: BusinessProfileRequest): Result<SaveBusinessProfileResponse> {
        return try {
            val response: HttpResponse = httpClient.post("${ApiConstants.BASE_URL}${ApiConstants.Endpoints.SAVE_BUSINESS_PROFILE}") {
                contentType(ContentType.Application.Json)
                setBody(profile)
            }
            if (response.status == HttpStatusCode.OK) {
                val result: SaveBusinessProfileResponse = response.body()
