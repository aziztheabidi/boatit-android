...
class FutureVoyagesRepo(private val httpClient: HttpClient) {
    suspend fun voyages(): Result<FutureBookedVoyages> {
        return try {
            val userId = AppConstants.USER_ID
            val response: HttpResponse = httpClient.get("${ApiConstants.BASE_URL}${ApiConstants.Endpoints.GET_FUTURE_BOOKED_VOYAGES}") {
                url {
