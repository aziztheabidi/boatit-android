...
class StartVoyageRepository(private val httpClient: HttpClient) {
    suspend fun status(profile: VoyageStartRequest): Result<VoyageStartResponse> {
        return try {
            val response: HttpResponse = httpClient.post("${ApiConstants.BASE_URL}${ApiConstants.Endpoints.START_VOYAGE}") {
                contentType(ContentType.Application.Json)
                setBody(profile)
            }
            if (response.status == HttpStatusCode.Created) {
                val result: VoyageStartResponse = response.body()
