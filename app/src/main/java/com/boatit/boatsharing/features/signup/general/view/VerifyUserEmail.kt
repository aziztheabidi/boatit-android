...
import com.boatit.boatsharing.features.signup.general.viewmodel.VerifyEmailUiEffect
import com.boatit.boatsharing.features.signup.general.viewmodel.VerifyEmailUiEvent
...
fun VerifyUserEmail(
    ...
    LaunchedEffect(Unit) {
        viewModel.uiEffect.collectLatest { effect ->
            when (effect) {
                is VerifyEmailUiEffect.ShowToast -> {
                    Toast.makeText(context, effect.message, Toast.LENGTH_SHORT).show()
                }

                is VerifyEmailUiEffect.NavigateToCreatePassword -> {
                    Log.e("tokenValue", effect.token)
                    navController.navigate(AccountRoutes.createAccountStepThree(effect.token))
                }
            }
        }
    }
            ...
            CustomButton(
                ...
                onButtonClick = {
                    if (isValidate) {
                        ...
                        viewModel.onEvent(VerifyEmailUiEvent.Submit(userEmail, otp))
                        isTimerEnabled = true
                    }
                },
            )
