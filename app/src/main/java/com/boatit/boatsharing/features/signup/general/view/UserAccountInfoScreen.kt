...
fun UserAccountInfoScreen(
    ...
    val isEmailValid =
        paypalEmail.isNotEmpty() &&
            android.util.Patterns.EMAIL_ADDRESS.matcher(paypalEmail).matches()
    ...
    LaunchedEffect(fetchUiState.errorMessage) {
        if (getingData && !fetchUiState.errorMessage.isNullOrBlank()) {
            Toast.makeText(context, fetchUiState.errorMessage, Toast.LENGTH_SHORT).show()
            getingData = false
        }
    }
